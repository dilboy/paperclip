...
const CLAUDE_TRANSIENT_UPSTREAM_RE =
  /(?:rate[-\s]?limit(?:ed)?|rate_limit_error|too\s+many\s+requests|\b429\b|overloaded(?:_error)?|server\s+overloaded|service\s+unavailable|\b503\b|\b529\b|high\s+demand|try\s+again\s+later|temporarily\s+unavailable|throttl(?:ed|ing)|throttlingexception|servicequotaexceededexception|out\s+of\s+extra\s+usage|extra\s+usage\b|claude\s+usage\s+limit\s+reached|5[-\s]?hour\s+limit\s+reached|weekly\s+limit\s+reached|usage\s+limit\s+reached|usage\s+cap\s+reached|socket\s+connection\s+was\s+closed\s+unexpectedly|socket\s+hang\s+up|connection\s+reset\s+by\s+peer|premature\s+close|\bECONNRESET\b|\bEPIPE\b|\bETIMEDOUT\b|\bEAI_AGAIN\b|\bENETUNREACH\b|\bEHOSTUNREACH\b|request\s+timed\s+out|request\s+timeout|failed\s+to\s+fetch|fetch\s+failed|network\s+request\s+failed|network\s+error|APIConnectionError|APITimeoutError)/i;
